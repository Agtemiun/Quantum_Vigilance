...
import pygame
import config
from Scripts.functional_blocks import functional_block,Jumper_block,Exit_block #,GazePlatform

NEIGBOURS = config.NEIGHBOURS
PHYSICS_TILES = config.PHYSICS_TILES
FUNC_BLOCKS = {"jumper"}
class Tilemap:
    ...
    def load(self,path):
        ...
        for tile in self.tilemap:
            match self.tilemap[tile]["type"]:
                ...
                #case "GazePlatform":
                #    self.functional_blocks.append(GazePlatform(self.tilemap[tile]["pos"], self.game))
    ...
    def fun_render(self, surf, shift):
        for fb in self.functional_blocks:
            # если это GazePlatform, передаём gaze_rect
            #if isinstance(fb, GazePlatform):
            #    fb.update(gaze_rect)
            #else:
            fb.update()
            fb.render()
