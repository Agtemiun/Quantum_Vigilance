...
"""
class GazePlatform(functional_block):
    def __init__(self, pos, game):
        super().__init__("GazePlatform", pos, game)
    def update(self, gaze_rect):
        # глобальная позиция на экране
        screen_x = self.pos[0] - self.game.camera_shift[0]
        screen_y = self.pos[1] - self.game.camera_shift[1]
        tile_rect = pygame.Rect(
            screen_x, screen_y,
            config.TILE_SIZE, config.TILE_SIZE
        )
        # активируется, только если фокус пересекается с тайлом
        self.active = gaze_rect.colliderect(tile_rect)
"""