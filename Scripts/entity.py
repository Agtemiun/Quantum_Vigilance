...
class PhysicsEntity:
    def __init__(self, game, e_type, pos, size):
        ...
        self.flip = False
        self.action = ""
        self.anim_offset = config.ANIM_OFFSET
    ...
    def update(self,tilemap, movement=(0,0), gravity=config.GRAVITY,dash = False):
        self.collisions = {"up": False,"down": False,"left": False,"right": False}
...
class Player(PhysicsEntity):
    ...
    def update(self,tilemap,movement=(0,0)):
        ...
        if self.air_time>config.WALL_GRAB_THRESHOLD:
            self.set_action('jump')

        if self.collisions["right"] or self.collisions["left"] and self.air_time>config.WALL_GRAB_THRESHOLD:
            self.wall_grap =False
    ...
    def jump(self):
        ...
        if self.wall_grap == True:
            ...
            if self.collisions["right"]:
                self.stamina-=config.WALL_JUMP_STAMINA_COST 
                self.velocity[1] = config.JUMP_VELOCITY
                self.velocity[0] = -config.WALL_JUMP_VELOCITY_X
    ...
    def Phantom_dash(self):
        if self.dashing==0 and self.stamina>0:
            self.stamina-=config.DASH_STAMINA_COST 
            if self.flip:
                self.dashing = -config.DASH_POWER
            else: 
                self.dashing = config.DASH_POWER
            self.dash =True